class ApplyRecordsController < ApplicationController
  ...
  def index
    ...
    if current_user.job=="客服人员"
      ...
      @apply_records=ApplyRecord.where(:itemsave => current_user.username, :state =>["物品交接中" ,"物品已入库","物品送往备案","采样中","采样完毕"]).page(params[:page]).order(:created_at=>:desc)
    elsif current_user.job=="作品备案认证人员"
      @apply_records=ApplyRecord.where(:sample => current_user.username,  :state =>["采样中"]).page(params[:page]).order(:created_at=>:desc)
    elsif current_user.job=="草拟与证书发送人员"
      @apply_records=ApplyRecord.where(:issue => current_user.username,  :state =>["证书签发中"]).page(params[:page]).order(:created_at=>:desc)
